...
function CostsPage() {
  ...
  const openEdit = (row: Ingredient) => { setEditing(row); setOpen(true); };

  return (
    <AppShell>
      <header className="mb-4 flex items-end justify-between">
        <div>
          <h1 className="text-2xl font-black tracking-tight">Ingredient Library</h1>
...
function IngredientSheet({
  ...
  // Auto-fill density when name or unit changes (only if user hasn't manually overridden)
  useEffect(() => {
    if (!isWeight) return;
    ...
    if (auto != null) {
      ...
      // name no longer matches table — clear table-sourced value
  ...
  const preview = useMemo(() => {
    ...
    const cpi = costPerItem(price, qty);
    const cpf = costPerFlOz({
      package_price: price, package_qty: qty, item_size: size,
      unit: form.unit, density: dens,
    });
    return { cpi, cpf };
  }, [form]);
