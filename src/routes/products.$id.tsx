...
/* ---------------- Produce (Production Calculator) ---------------- */

const QT_TO_ML = 946.353;
const BATCH_DENSITY = 1.05; // g/ml
const G_PER_QT = QT_TO_ML * BATCH_DENSITY; // 993.67
const FLOZ_TO_ML = 29.5735;
const OZ_TO_G = 28.3495;
const INGREDIENT_DENSITY_FLOZ = 1.03; // g/ml fallback for fl oz packs
...
function ProduceSection({
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem("cocobliss_blender_qt");
      if (stored) setBlenderQt(stored);
    } catch { /* ignore */ }
  }, []);

  useEffect(() => {
    if (!blenderQt) return;
    try { localStorage.setItem("cocobliss_blender_qt", blenderQt); } catch { /* ignore */ }
  }, [blenderQt]);
  ...
  const rows = useMemo(() => {
    return formula.ingredients.map((fi) => {
      const ing = ingredients.find((i) => i.id === fi.ingredient_id) as IngredientWithMeta | undefined;
  ...
  }, [formula.ingredients, ingredients, totalG, blenderG]);

  const ready = target > 0 && blender > 0;
                ...
                <div key={r.key} className="rounded-2xl border border-border bg-card p-4">
                  <div className="mb-2 flex items-baseline justify-between gap-2">
                    <div className="font-semibold">
                      ...
                      {(r.ing as IngredientWithMeta | undefined)?.library_code && (
                        <span className="ml-2 text-xs font-normal text-muted-foreground">
                          {(r.ing as IngredientWithMeta).library_code}
                        </span>
                      )}
                    </div>
                  </div>
